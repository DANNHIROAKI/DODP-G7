...
def PreprocessNewbook(books_path, ratings_path, users_path):
    print("PERT THREE: Preprocessing NewBook's Data")
    ...
    merged_data.to_csv('Data-Files\\BX-NewPreprocessed.csv', index=False)
    print(merged_data.head())
    return merged_data

# 1. Data Loading
def Load_data(books_path, ratings_path, users_path):
    print("Preprocessing_NewBook # 1. Load the data from the CSV files")
...
def Preprocess_users(users):
    print("Preprocessing_NewBook # 2. Preprocessing Users")
...
def Preprocess_users_location(users):
    print("Preprocessing_NewBook # 2.1. Preprocessing User-Location")
...
def Preprocess_users_location_city(users):
    print("Preprocessing_NewBook # 2.1.1. Preprocessing User-City")
...
def Preprocess_users_location_country_clean(users):
    print("Preprocessing_NewBook # 2.1.2.1. Cleaning of User-Country non-missing values")
...
def Preprocess_users_location_country_fillna(users):
    print("Preprocessing_NewBook # 2.1.2.2. Filling in missing values for User-Country")
...
def Preprocess_users_location_country_fillna_map(users):
    print("Preprocessing_NewBook # 2.1.2.2.1. Create a mapping of State——Country")
...
def Preprocess_users_location_country_fillna_fill(users, state_to_country):
    print("Preprocessing_NewBook # 2.1.2.2.2. Fill in missing Country values based on mapping")
...
def Preprocess_users_location_state(users):
    print("Preprocessing_NewBook # 2.1.3. Preprocssing User-States")
...
def Preprocess_users_location_state_clean(users):
    print("Preprocessing_NewBook # 2.1.3.1. Cleaning of User-State non-missing values")
...
def Preprocess_users_location_state_percentage(users):
    print("Preprocessing_NewBook # 2.1.3.2. Calculate the percentage of users in each state / country")
...
def Preprocess_users_location_state_generate(users, valid_states_per_country):
    print("Preprocessing_NewBook # 2.1.3.3. Generate User-Location columns based on the logic of the flowchart") 
...
def Preprocess_users_location_state_top100(users):
    print("Preprocessing_NewBook # 2.1.3.4. Select the top 100 User-Locations")
...
def Preprocess_users_location_encoder(users):
    print("Preprocessing_NewBook # 2.1.4. Encoding: Converting the Location text into numerical values")
...
def Preprocess_users_age(users):
    print("Preprocessing_NewBook # 2.2. Preprocessing User-Age")
...
def Preprocess_users_age_clean(users):
    print("Preprocessing_NewBook # 2.2.1. Cleaning of User-Age non-missing values")
...
def Preprocess_users_age_fillna(users):
    print("Preprocessing_NewBook # 2.2.2. Filling in missing values for User-Age")
...
def Preprocess_users_age_outliers(users):
    print("Preprocessing_NewBook # 2.2.3. Handling outliers in User-Age")
...
def Preprocess_users_age_binning(users):
    print("Preprocessing_NewBook # 2.2.4. Binning User-Age")
...
def Preprocess_books(books):
    print("Preprocessing_NewBook # 3. Preprocessing Books")
...
def Preprocess_books_publication(books):
    print("Preprocessing_NewBook # 3.1. Preprocessing Year-Of-Publication")
...
def Preprocess_books_publication_outliers(books):
    print("Preprocessing_NewBook # 3.1.1. Handling of Publication Outliers")
...
def Preprocess_books_publication_binning(books):
    print("Preprocessing_NewBook # 3.1.2. Binning Year-Of-Publication")
...
def Preprocess_books_publication_encoder(books):
    print("Preprocessing_NewBook # 3.1.3. Encoder Year-Of-Publication-Binned")
    ordinal_encoder = OrdinalEncoder()
...
def Encode_books_PublisherAuthor(books):
    print("Preprocessing_NewBook # 3.2. Encoding Publisher & Author")
    ordinal_encoder = OrdinalEncoder()
...
def Preprocess_books_title(books):
    print("Preprocessing_NewBook # 3.3. Book-Title Preprocessing")
...
def Preprocess_books_title_clean(books):
    ...
    def Preprocess_text_spacy(text):
        ...
        return clean_text
    tqdm.pandas(desc="Preprocessing_NewBook # 3.3.1. Cleaning the titles (To create word cloud)") 
...
def Preprocess_books_title_wordcloud(books):
    print("Preprocessing_NewBook # 3.3.2. Create Word Cloud (In ./Plots_NewBook)")
    wordcloud = WordCloud(
        width=1800, 
        height=1400, 
    ...
    plt.axis('off')  
    plt.savefig('Plots_NewBook\\wordcloud.png')
    plt.close()

# 3.3.3. Word➡️Vector: BERT based method
def Preprocess_books_title_vector(books):
    print("Preprocessing_NewBook # 3.3.3. Word to Vector: BERT based method")
...
def Preprocess_books_title_vector_loadmodel():
    print("Preprocessing_NewBook # 3.3.3.1. Load pretrained model")
...
def Preprocess_books_title_vector_GPU(model):
    print("Preprocessing_NewBook # 3.3.3.2. Trying to move the pretrained model to GPU")
...
def Preprocess_books_title_vector_batch(
    ...
    print("Preprocessing_NewBook # 3.3.3.3. Batch text processing")
    batch_size = 32
    all_embeddings = []
    total_batches = (len(books['Book-Title']) + batch_size - 1) // batch_size
    progress_bar = tqdm(
        total=total_batches, 
        desc="Preprocessing_NewBook # 3.3.3.4. Training BERT Model"
    )
...
def Preprocess_books_title_DimensionReduct(books):
    print("Preprocessing_NewBook # 3.3.4. UMAP Baesd Vector Dimension Reduction")
    reducer = umap.UMAP(
        n_neighbors=15, 
        n_components=1, 
        metric='cosine'
...
def Preprocess_books_final(books):
    print("Preprocessing_NewBook # 3.4. Merge Book Data")
    columns_to_keep = [
        'ISBN', 
        'Year-Of-Publication', 
...
def Preprocess_ratings(ratings):
    print("Preprocessing_NewBook # 4. Preprocessing Ratings")
...
def Preprocess_final(
    ...
    print("Preprocessing_NewBook # 5. For All the Data: Merge & Review")
...
def Preprocess_final_merge(
    ...
    print("Preprocessing_NewBook # 5.1. Merge")
...
def Preprocess_final_correlation(merged_data):
    print("Preprocessing_NewBook # 5.2. Correlation (In ./Plots_NewBook)")
    ...
    plt.tight_layout()
    plt.savefig('Plots_NewBook\\correlation.png')
    plt.close(fig)
...
def Preprocess_final_correlation_pearson(merged_data):
    ...
    print("Preprocessing_NewBook # 5.2.1. Linear relationship correlation (Pearson) ")
...
def Preprocess_final_correlation_mutualinfo(merged_data):
    ...
    merged_data = merged_data.drop(columns=['ISBN'])
    print("Preprocessing_NewBook # 5.2.2. Non-linear relationship correlation (Mutual Information)")
    X = merged_data
